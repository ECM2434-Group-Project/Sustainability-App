...
'''
    ...
    def post(self, request):
        ...
        # Update users score
        if score != 0:
            UserModel.objects.filter(id=user.id).update(score=user.score + score)
        ...
        with transaction.atomic():
            try:
                ...
                if claimSerializer.is_valid(raise_exception=True):
                    ...
                    # update user score if they have claimed a bag
                    UserModel.objects.filter(id=user.id).update(score=user.score + 3)
